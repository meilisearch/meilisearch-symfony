...
final class MeilisearchManager implements SearchManagerInterface
{
        ...
        foreach ($searchable as $entity) {
            ...
            foreach ($matchingIndices as $indexConfig) {
                ...
                $batches[$indexName][] = new SearchableObject(
                    ...
                    $indexConfig['primary_key'],
                    $entity,
                    $this->getSingleIdentifier($provider, $entity), // @todo: extension point?
                    $this->normalizer,
                    ['groups' => []]
                );
            }
    ...
    public function search(
        ...
        $identifiers = array_column($hits, $index['primary_key']);
        ...
        foreach ($hits as $hit) {
            $documentId = $hit[$index['primary_key']];
        ...
        foreach (array_chunk($items, $this->configuration->get('batchSize')) as $chunk) {
            ...
            foreach ($chunk as $item) {
                ...
                $searchableChunk[] = new SearchableObject(
                    ...
                    $indexConfig['primary_key'],
                    $object,
                    $this->getSingleIdentifier($provider, $originalSource), // @todo: extension point?
                    $this->normalizer,
                    $normalizationContext,
                );
            }
    ...
    private function getSingleIdentifier(DataProviderInterface $provider, object $object): string|int
    {
        $ids = $provider->getIdentifierValues($object);

        if (1 === \count($ids)) {
            $identifier = reset($ids);
